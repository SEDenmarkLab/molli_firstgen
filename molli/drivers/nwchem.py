...
class AsyncNWCHEMDriver(AsyncExternalDriver):
    ...
    async def optimize_esp(
        ...
        nw_lines = self._nwchem_composition(xyz_=xyz_lines,chg = chg, opt_=opt)
        ...
        with open(self.name+'espout.txt','a') as g:
            g.write(mol.name+files[f"{nn}.esp"]+'\n\n\n')
    ...
    def _nwchem_composition(self,xyz_,chg,opt_=True):
        '''
