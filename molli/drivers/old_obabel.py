...
class OpenBabelDriver(ExternalDriver):
    ...
    def convert(self, mol_text: str, *args, src: str = "pdb", dest: str = "mol2"):
        """
    ...
    def minimize(
        ...
        """
        prefix = f"obm.{os.getpid()}"
        with open(os.path.join(self.cwd, f"{prefix}.{src}"), "wt") as f:
            f.write(mol_text)

        pdb = self("obminimize", "-ff", ff, "-n", n, "-c", c, f"{prefix}.{src}")
    ...
    def hadd_opt(
        ...
        xyz_h_opt = self.minimize(mol2_h, src="mol2", dest="xyz", ff=ff, n=n, c=c)
